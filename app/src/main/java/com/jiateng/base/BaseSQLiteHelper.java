...
public abstract class BaseSQLiteHelper<T> extends SQLiteOpenHelper {
     ...
     */
    @SneakyThrows
    protected int insert(T t, String tableName) {
        ...
        return (int) wdb.insert(tableName, null, values);
    }
    ...
    protected int deleteWithTableName(String tableName, String whereClause, Object... args) {
        String[] params = null;
        if (args != null) {
            params = Arrays.stream(args).map(o -> o + "").collect(Collectors.toList()).toArray(new String[args.length]);
        }
        return wdb.delete(tableName, whereClause, params);
    }
